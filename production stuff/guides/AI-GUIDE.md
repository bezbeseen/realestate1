...
### Current Features:
- ✅ **110 total pages** (38 products, 20 services, 9 locations, etc.)
...
- ✅ **Developer tools** - 14+ magic testing/diagnostic tools
- ✅ **Location pages** - 9 Bay Area cities for local SEO

## 🗺️ Sitemap.xml - CRITICAL Single Source of Truth

### **WHY SITEMAP IS CRITICAL:**
The `sitemap.xml` file is **THE SINGLE SOURCE OF TRUTH** for the entire website structure. It is automatically generated during the build process and serves multiple critical functions:

### **What Uses Sitemap:**
1. ✅ **Developer Tools** - ALL 14 magic buttons pull page lists from sitemap
   - Analytics checker, link checker, image validator, SEO report, etc.
   - Stats dashboard auto-updates with real counts from sitemap
2. ✅ **Search Engines** - Google, Bing, etc. use it to discover all pages
3. ✅ **Build Validation** - Confirms all pages generated successfully
4. ✅ **Site Statistics** - Accurate page counts for reporting

### **Sitemap Generation:**
- **Location:** `generated/sitemap.xml` (auto-generated, never edit manually)
- **Generated by:** `build/build.js` during build process
- **Current count:** 110 URLs (all pages on the site)
- **Format:** Standard XML sitemap protocol with lastmod, changefreq, priority

### **CRITICAL RULES:**
- ⚠️ **NEVER edit sitemap.xml manually** - it's regenerated on every build
- ✅ **Always rebuild after adding/removing pages** - sitemap updates automatically
- ✅ **Developer tools load sitemap dynamically** - no hardcoded page lists
- ⚠️ **If sitemap is broken, developer tools won't work**

### **How to Verify Sitemap:**
```bash
# Check sitemap exists and has correct count
grep -c "<loc>" generated/sitemap.xml
# Should show: 110

# View sitemap structure
head -50 generated/sitemap.xml
```

### **Adding New Pages:**
When you add a new page type (product, service, location, etc.):
1. Add to appropriate data file (`products.json`, `locations.json`, etc.)
2. Run build: `node build/build.js`
3. ✅ Sitemap automatically updates with new page
4. ✅ Developer tools automatically see new page
5. ✅ Search engines discover new page on next crawl

**No manual sitemap editing needed - it's 100% automated!**
